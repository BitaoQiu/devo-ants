...
# Using M. pharaonis 2nd instar larvae as an example [validation].
train_age = '2nd' # Stage with unknown caste identities.
test_age = '3rd' # Stage with known caste identities.
...
# Step 0: Identificaton of caste DEGs at 3rd instar as features (candidate genes). 
# Note: One can also use whole transcriptome as features, but the signal to noise ratio will be lower.
...
reverse_pca.exp = function(train_data.exp, test_data.exp, train_age, test_age, sample.info,
  ...
  batch = droplevels(filter_table$age) # We consider age as a co-variate. If correct_dev == T,we normalise the mean expression levels across age.
  ...
  if (correct_dev != T) # If correct_dev == F, we don't normalize the expression difference.
  {combat_edata = exp.filter}
  ...
  n = 10
  # Step 1.Construction of PCs from transcriptomes of samples with unknown caste identities.
  ...
  # Step 2. Projection of transcriptomes of samples with known caste identities onto the constructed PCs.
...
# Step 3.a: Visual examination of the PC value similarity between 2nd instar and 3rd instar larvae. 
